...
//        }
    ...
    private func setupPullToRefresh() {
        ...
        refreshControl.addTarget(self, action: "reset", for: .valueChanged, for#selector(UIGestureRecognizer.reset) .valueChanged)
    ...
    func refresh() {
        ...
        JobService.shared().fetch(page: page,
                                         rpp: rpp,
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "jobsTableViewCellReuseIdentifier", for: indexPath as IndexPath) as! JobTableViewCell
    ...
    func cancelJobAtIndexPath(indexPath: NSIndexPath) {
        ...
        if let cell = tableView.cellForRow(at: indexPath as IndexPath as IndexPath) as? JobTableViewCell {
            if ["canceled", "completed"].index(of: job.status) == nil {
                cell.dismiss()
                promptForJobCancellation(job: job, cell: cell)
            }
        }
    }

    func promptForJobCancellation(job: Job, cell: JobTableViewCell) {
        let preferredStyle: UIAlertControllerStyle = isIPad() ? .alertalert : .actionSheetactionSheet
        ...
        let cancelAction = UIAlertAction(title: "No, Don't Cancel", style: .cancelcancel) { action in
            cell.reset()
        }
        alertController.addAction(cancelAction)

        let setCancelJobAction = UIAlertAction(title: "Cancel Job", style: .destructivedestructive) { action in
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "JobWizardTabBarControllerSegue", sender: cell)
    ...
    func jobCreationViewController(viewController: JobCreationViewController, didCreateJob job: Job) {
        ...
        performSegue(withIdentifier: "JobWizardTabBarControllerSegue", sender: job)
    }
...
//
//            if shouldCancelJob {
//                jobsViewController?.cancelJobAtIndexPath(indexPath: indexPath as NSIndexPath)
//            } else {
//                restoreCell()
//            }
//
...
//                    self!.jobsViewController?.performSegue(withIdentifier: "JobWizardTabBarControllerSegue", sender: cell)
